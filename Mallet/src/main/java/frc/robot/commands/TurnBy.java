...
public class TurnBy extends CommandBase {
    ...
    public void execute() {
        ...
        if (distance > 180)
            distance = Math.abs(distance - 360.0f); // wrap around distance
        float speed = 2.0f * Math.max(Constants.K_MIN_TURNING_SPEED,
                Math.min(Constants.K_MAX_TURNING_SPEED, (.01f * (distance - 10.0f) + .2f))); // set speed dynamically to
