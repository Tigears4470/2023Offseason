...
public class DriveOffPlatform extends CommandBase {
  ...
  public void execute() {
    ...
    // System.out.println(Math.abs(m_drivetrain.getAverageDistanceInch() -
    // startingEncoderVal));
  ...
  public boolean isFinished() {
    ...
    return (Math.abs(m_gyro.getAngleX()) < 1.0)
        && (Math.abs(m_drivetrain.getAverageDistanceInch() - startingEncoderVal) > 8 * Constants.K_TICKS_PER_FEET);
  }
}
